...
const Delete = ({ message, id }) => {
	...
	const showDeleteForm = async () => {
		setShow(true);
		try {
			const response = await fetchOne("project", id);
	...
	const deleteYesClick = async () => {
		try {
			const response = await deleteOne("project", id);
	...
	return (
		<>
					...
					<div className="w-11/12 md:w-1/2 mx-auto mb-10 bg-white border-2 border-gray-300 rounded-md shadow-md duration-300">
						...
						<div className="p-4 lg:p-6 flex flex-col space-y-4">
							<div className="w-full">
								...
								<h1 className="text-center text-gray-600 font-bold">{data.name}</h1>
							</div>
			...
			<button onClick={showDeleteForm} title="Delete" className="w-8 h-8 rounded-full hover:bg-gray-200 mr-3 flex justify-center items-center">
